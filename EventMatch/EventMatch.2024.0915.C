...
#include "/home/kal-dc-ana/EXP/TRIUMF/2024/ANA/EventMatch/EventMatch.2024.0915.h"
#include <iostream>
#include <fstream>
#include <iomanip>
#include <cmath>
...
void EventMatch::DC_TS_Match(int runN=0, int subrunN=0)
{
  ...
  TString DC_name  = Form("%s/run_%04d_%04d.dat.root",  DC_path.Data(), runN, subrunN);
  TString NIM_name = Form("%s/MSE%06d.root"          , NIM_path.Data(), runN);
  TString out_name = Form("%s/run_%04d_Sync.root"    , out_path.Data(), runN);
  ...
  ofstream ofTSmon(Form("/home/kal-dc-ana/EXP/TRIUMF/2024/ANA/TXT/TSmon_run%06d.txt",runN));
...
void EventMatch::DC_TS_Fit(int runN=0)
{
  ...
  TString Sync_name = Form("%s/run_%04d_Sync.root", Sync_path.Data(), runN);
  ...
  ofstream ofPara(Form("/home/kal-dc-ana/EXP/TRIUMF/2024/ANA/TXT/TS_Fit_para_run%06d.txt",runN));
