...
{
  TString merge_path = ".";
  TString merge_name = Form("%s/run_%04d.root", merge_path.Data(), runN);
  TFile *merge_file  = TFile::Open(merge_name);
  TTree *tree_merge  = (TTree*)merge_file->Get("tree");
  if (!merge_file || !merge_file->IsOpen()) {
    std::cerr << "Error: Could not open file " << merge_name << std::endl;
    return;
  }
  ofstream ofPara(Form("./txt/TS_Fit_para_run%d.txt",runN));
  
  Long64_t nentries = tree_merge->GetEntries();
  ...
  tree_merge->SetBranchAddress("TS_NIM",     &TS_NIM);
  tree_merge->SetBranchAddress("dTS_NIM",    &dTS_NIM);
  tree_merge->SetBranchAddress("TS_DC",      &TS_DC);
  tree_merge->SetBranchAddress("dTS_DC",     &dTS_DC);
  tree_merge->SetBranchAddress("TS_diff_DC", &TS_diff_DC);
  tree_merge->SetBranchAddress("dTS_diff_DC",&dTS_diff_DC);
  /*
...
void EventMatch::DC_TS_Calibration(int runN=0, int subrunN=0)
{
  ...
  TString out_path = ".";
  TString DC_name = Form("%s/run_%04d/run_%04d_%04d.dat.root", DC_path.Data(), runN, runN, subrunN);
  TString out_name = Form("%s/run_%04d_calib.root", out_path.Data(), runN);
  TFile *file_DC = TFile::Open(DC_name);
  TFile *outfile = new TFile(out_name,"RECREATE");
  if (!file_DC || !file_DC->IsOpen()) {
    std::cerr << "Error: Could not open file " << DC_name << std::endl;
  ...
  tree->Branch("TS_DC_pre",&TS_DC_pre,"TS_DC_pre/D");
  tree->Branch("dTS_DC",&dTS_DC,"dTS_DC/D");
  tree->Branch("TS_DC_calib",&TS_DC_calib,"TS_DC_calib/D");
  //tree->Branch("TS_diff_DC_calib",&TS_diff_DC_calib,"TS_diff_DC_calib/D");
...
void EventMatch::TS_Event_Matching(int runN=0, Double_t accuracy_ns=100.0){
  Double_t accuracy_sec = accuracy_ns * 1e-9;
  
  TString DC_path  = ".";
  TString NIM_path = "../ROOT/test_0808";
  TString out_path = ".";
  TString DC_name  = Form("%s/run_%04d_calib.root",   DC_path.Data(),  runN       );
  TString NIM_name = Form("%s/run_%d/MSE0000%d.root", NIM_path.Data(), runN, runN );
  TString out_name = Form("%s/run_%04d_match.root",   out_path.Data(), runN       );
  TFile *file_DC   = TFile::Open(DC_name);
  TFile *file_NIM  = TFile::Open(NIM_name);
  TFile *outfile   = new TFile(out_name,"RECREATE");
  ...
  tree->Branch("triggerTimeStamp", &triggerTimeStamp, "triggerTimeStamp/l");
  tree->Branch("TS_DC",&TS_DC,"TS_DC/D"); // convert ns->sec
  tree->Branch("TS_DC_calib",&TS_DC_calib,"TS_DC_calib/D"); // convert ns->sec
...
void EventMatch::TS_Fit(int IP=0)
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(jentry % pickup_interval == 0){
      start = jentry;
      end = start + pickup_interval - 1;
      if(end > nentries) end = nentries;
      for(int i=start;i<end;i++){
        LoadTree(i);
        fChain->GetEntry(i);
        if(i % int(pickup_interval*0.01) == 0) TS.push_back(TS_Kal[IP]);
        if(i % int(pickup_interval*0.01) == 0) dTS.push_back((*TS_diff)[IP]);
      }
...
void EventMatch::TS_Calibration(int IP=0)
{
  ...
  TFile *newFile = TFile::Open("../ROOT/test_0808/run_7/MSE000007.root","UPDATE");  
  TTree *newTree = new TTree("newTree","newTree");
  hTS_calib_diff[IP] = new TH2F(Form("hTS_calib_diff_%d",IP),Form("TS_{Kalliope calib}(%02d) - TS_{NIM};TS_NIM (s);TS_{Kalliope calib}(%02d) - TS_{NIM} [s]",IP,IP),1000,0,60,1000,-1e-6,1e-6);
  newTree->Branch("TS_NIM",       &TS_NIM,      "TS_NIM/D"           );
  ...
  newTree->Write();
  hTS_calib_diff[IP] -> Write();
  //tree->Write("",TObject::kOverwrite);
  newFile->Close();  

  TFile *f1 = TFile::Open("../ROOT/test_0808/run_7/MSE000007.root");
