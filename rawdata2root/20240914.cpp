...
void rawdata2root(int runN = 10, int N_IP = 0, bool fNIM = 0, bool ftree = 0,
  ...
  ofstream positronfile(Form("./TXT/%s/MSE%06d_%02d.csv", "POSITRON",   runN, SCAN_N));
  ...
  for (int ii = 0; ii < N_IP; ii++) {
    cout << Form("path: %s, IP: %d, base_name: %s", path.c_str(), vecIP[ii], base_name.Data()) << endl;
  ...
  if (!fNIM) {
    cout << Form("ifname[0]: %s", ifname[0].c_str()) << endl;
  ...
  int Fiber_L_all[2][2][2][64][hitNmax];
  int Fiber_num[2][2][2] = {}; // Count detected channel to check whether Trackable or not
  int Fiber_num_ch[12][32] = {0};
  ...
  int N_trackable = 0;
  int N_Layer[2][2][2] = {0}; // xy | ud | oi
  int N_Positron[2] = {0}; // ud 
  ...
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      for (int k = 0; k < 2; k++)
        for (int l = 0; l < 64; l++) 
          for (int m = 0; m < hitNmax; m++) Fiber_L_all[i][j][k][l][m] = 0;
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      for (int k = 0; k < 2; k++)
  ...
  #ifdef TRACKING_ON
  TH2F * hFiber_in[2];
  hFiber_in[0] = new TH2F("hFiber_in_up", "hFiber_in_up; x[mm]; y[mm]", 72, -36, 36, 72, -36, 36);
  hFiber_in[1] = new TH2F("hFiber_in_dn", "hFiber_in_dn; x[mm]; y[mm]", 72, -36, 36, 72, -36, 36);
  TH2F * hFiber_out[2];
  hFiber_out[0] = new TH2F("hFiber_out_up", "hFiber_out_up; x[mm]; y[mm]", 72, -36, 36, 72, -36, 36);
  hFiber_out[1] = new TH2F("hFiber_out_dn", "hFiber_out_dn; x[mm]; y[mm]", 72, -36, 36, 72, -36, 36);
  ...
  size_t READ_SIZE_NIM = 5 * 1024 * 1024 ;  
  ...
  cout << "fsize_nimtdc: " <<  fsize_nimtdc << endl;
  bool notONLINE_FLAG = false;

  if (!ONLINE_FLAG) {
    READ_SIZE_NIM = static_cast<double>(fsize_nimtdc) - 1000 * 1024;
    cout << "READ_NIM_SIZE: " << READ_SIZE_NIM << endl;
    READ_SIZE_NIM = ( READ_SIZE_NIM / 1024 ) * 1024;
    cout << "READ_NIM_SIZE (modified): " << READ_SIZE_NIM << endl;
    ONLINE_FLAG = true;
    notONLINE_FLAG = true;
  }
  ...
  if (ONLINE_FLAG) {
      ...
      start_pos_nimtdc = (start_pos_nimtdc / 1024) * 1024;
  ...
  for (int nIP = 0; nIP < N_IP; nIP++) { 
      ...
      if (ONLINE_FLAG) {
          ...
          double KAL_RATIO = READ_RATIO;
          streampos start_pos = fsize * KAL_RATIO - 500 * 1024;
  ...
  while (WHOLE_FLAG) {
    ...
    if (fNIM) {
      ...
      while (!rawdata_nimtdc.eof()  && !Stop_NIM ) {
	    ...
	    ch == 2 ? N_Positron[0] ++ : N_Positron[0] += 0; // up
	    ch == 3 ? N_Positron[1] ++ : N_Positron[1] += 0; // dn
          }
          Traw_NIM_num[1][ch]++;
        }
    ...
    for (int nIP = 0; nIP < N_IP; nIP++) {
      ...
      while (!rawdata[nIP].eof()  && !Stop_KAL[nIP] ) {
        ...
        */
              ...
              for (int j = 0; j < 2; j++) { // up or dpwn
                for (int k = 0; k < 2; k++) { // in or out
                  for (int l = 0; l < 64; l++) { // ch
                    Fiber_L[i][j][k][l] = -1e4;
                    for (int m = 0; m < hitNmax; m++) Fiber_L_all[i][j][k][l][m] = -1e4;
                    Fiber_T[i][j][k][l] = -2e4;
		...
		if (abs(Fiber_L[xy][ud][oi][fiber_ch] - Traw_NIM_L_valid[ud + 2] - (-1400)) < 100) {
		  Fiber_num[xy][ud][oi] ++;
		  Fiber_num_ch[nIP][ch] ++;
		}
    ...
    if (FILL_FLAG && !SKIP_FLAG_new) {
      ...
      for (int ii = 0; ii < 32; ii++) {
        for (int jj = 0; jj < hitNmax; jj++) {
          if (ii < 4) if (dTS_NIM > 1e-5) hNIM_L[ii] -> Fill(Traw_NIM_L[ii][jj]);
      ...
      for (int ud = 0; ud < 2; ud++) 
	    ...
	    if (Fiber_num[xy][ud][oi] == 0) continue;
      ...
      //====== Judge whether Trackable or not ======
      ...
      for (int ud = 0; ud < 2; ud++) {
        for (int xy = 0; xy < 2; xy++) {
          for (int oi = 0; oi < 2; oi++) {
            ...
            if (Fiber_num[xy][ud][oi] == 1) {
              Fiber_FLAG[xy][ud][oi] = true;
              for (int ch = 0; ch < ch_max; ch++) { 
                if ( abs(Fiber_L[xy][ud][oi][ch] - Traw_NIM_L_valid[ud + 2] - (-1400)) < 100 ) {
		  ...
		  break;
            ...
            if (Fiber_num[xy][ud][oi] == 2) {
              Fiber_FLAG[xy][ud][oi] = true;
              for (int ch = 0; ch < ch_max; ch++) { 
                if ( abs(Fiber_L[xy][ud][oi][ch] - Traw_NIM_L_valid[ud + 2] - (-1400)) < 100 
	    	  && abs(Fiber_L[xy][ud][oi][ch+1] - Traw_NIM_L_valid[ud + 2] - (-1400)) < 100) {
                  hValid[ud][xy][oi] -> Fill(ch, Fiber_L[xy][ud][oi][ch] - Traw_NIM_L_valid[ud + 2]);
                  hValid[ud][xy][oi] -> Fill(ch+1, Fiber_L[xy][ud][oi][ch+1] - Traw_NIM_L_valid[ud + 2]);
                  a[xy][oi] = (double) ch + 1.0 - ((double) ch_max / 2.0);
		  break;
                }
              }
            }
      ...
      if (Tracking_FLAG[0] == true) ud = 0;
      if (Tracking_FLAG[1] == true) ud = 1;
...
cAssym_subBG -> cd(1);
SetMargins();
Assym_FB_subBG->Rebin(4);
Assym_FB_subBG->Draw("E");  // "E" オプションを追加してエラーバーを表示
gPad -> SetLogy(0);
gPad -> Update();
...
cAssym_subBG -> cd(2);
SetMargins();
Assym_UD_subBG->Rebin(4);
Assym_UD_subBG->Draw("E same");  // "E" オプションを追加してエラーバーを表示
gPad -> SetLogy(0);
gPad -> Update();
...
cAssym_subBG -> Write();
  ...
  */
  ...
  //==== Valid events (e+) =================================================== 
  TCanvas * cPositron = new TCanvas(Form("cPositron"), Form("cPositron"), 1200, 600);
  cPositron -> Divide(4, 2);
  ...
  for (int ud = 0; ud < 2; ud ++) {
    for (int xy = 0; xy < 2; xy ++) {
       for (int oi = 0; oi < 2; oi ++){
          ...
          gPad -> SetLogz(1); gPad -> Update();
          cPositron -> cd(ii + 1) -> Modified();
          cPositron -> cd(ii + 1) -> Update();
       }
    }
  }
  cPositron -> Write();
  ...
  double rate_track = (double) N_trackable / N_event[0];
  ...
  cout << "--------------------------------------------------------------------------------" << endl;
  cout << "UP Counter      : " << Form("%8d events", N_Positron[0]                                                            )          << endl;
  cout << "X | UP | OUTER  : " << Form("%8d events | %6.2f", N_Layer[0][0][0], (double) N_Layer[0][0][0] / N_Positron[0] * 100) << " % " << endl;
  cout << "Y | UP | OUTER  : " << Form("%8d events | %6.2f", N_Layer[1][0][0], (double) N_Layer[1][0][0] / N_Positron[0] * 100) << " % " << endl;
  cout << "X | UP | INNER  : " << Form("%8d events | %6.2f", N_Layer[0][0][1], (double) N_Layer[0][0][1] / N_Positron[0] * 100) << " % " << endl;
  cout << "Y | UP | INNER  : " << Form("%8d events | %6.2f", N_Layer[1][0][1], (double) N_Layer[1][0][1] / N_Positron[0] * 100) << " % " << endl;
  cout << "DN Counter      : " << Form("%8d events", N_Positron[1]                                                            )          << endl;
  cout << "X | DN | OUTER  : " << Form("%8d events | %6.2f", N_Layer[0][1][0], (double) N_Layer[0][1][0] / N_Positron[1] * 100) << " % " << endl;
  cout << "Y | DN | OUTER  : " << Form("%8d events | %6.2f", N_Layer[1][1][0], (double) N_Layer[1][1][0] / N_Positron[1] * 100) << " % " << endl;
  cout << "X | DN | INNER  : " << Form("%8d events | %6.2f", N_Layer[0][1][1], (double) N_Layer[0][1][1] / N_Positron[1] * 100) << " % " << endl;
  cout << "Y | DN | INNER  : " << Form("%8d events | %6.2f", N_Layer[1][1][1], (double) N_Layer[1][1][1] / N_Positron[1] * 100) << " % " << endl;
  cout << endl;

  if (ONLINE_FLAG){
    ofstream csv;
    const TString csvname = "./CSV/statistic_summary.csv";
    csv.open(csvname, ios::app);
    if (csv.is_open()){
      csv << Form("%4d", runN) << ", "
	  << Form("%10.0f", (double) N_event[0]/(1-READ_RATIO)) << ", "
	  << Form("%8.0f", SkipN) << ", "
	  << Form("%9.0f", (double) N_trackable/(1-READ_RATIO)) << ", "
	// UP
	  << Form("%7d", N_Positron[0]) << ", "
	  << Form("%8d", N_Layer[0][0][0]) << ", "
	  << Form("%8d", N_Layer[1][0][0]) << ", "
	  << Form("%8d", N_Layer[0][0][1]) << ", "
	  << Form("%8d", N_Layer[1][0][1]) << ", "
	// DN
	  << Form("%7d", N_Positron[1]) << ", "
	  << Form("%8d", N_Layer[0][1][0]) << ", "
	  << Form("%8d", N_Layer[1][1][0]) << ", "
	  << Form("%8d", N_Layer[0][1][1]) << ", "
	  << Form("%8d", N_Layer[1][1][1]) << endl;
    }
  }
  ...
  cout << "--------------------------------------------------------------------------------" << endl;
  cout << "------ Rate of Positron (/10s) -------------------------------------------------" << endl;
  cout << "--------------------------------------------------------------------------------" << endl;
	cout << "  ";
  for (int jj = 0; jj < 32; jj++) cout << Form(",%4d", jj);
	cout << endl;
  for (int ii = 0; ii < N_IP; ii++) {
			cout << Form("%2d", vecIP[ii]);
			positronfile << Form("%2d, ", vecIP[ii]); 
			for (int jj = 0; jj < 32; jj++) {
          double value = 10. * (double)Fiber_num_ch[ii][jj] / Trun_total;
          int exponent = value == 0 ? 0 : (int) floor(log10(value));
          double mantissa = value / pow(10, exponent);
					//if(exponent < 0) {
					//		mantissa = 0;
					//		exponent = 0;
					//}
          cout      << Form(" ,%1.0fe%1d", mantissa, exponent);
          positronfile << Form("%6.2f", value);
					if (jj < 31) positronfile << ",";
			}
			cout << endl;
			positronfile << endl;
	}
...
void Check_Real_CH() {
  ...
  for (int i = 0; i < N_IP; i++) {
    ...
    ifname[i] = Form("../RAW/KAL_RUN/MSE000043_192.168.10.%d.rawdata", nIP);
...
void EstimateStat(){
  for(int runN=33; runN<64; runN++){
    rawdata2root(runN, 0, 0, 0, "test", true, false, 0);
  }
}
