...
#index_html_name="index_$(basename "$source_dir")"
...
# Iterate over each directory in the source_dir and add links to the index files
for dir in $(find "$default_source_dir" -mindepth 1 -maxdepth 1 -type d); do
  index_file="$dir/index.html"
  if [ -f "$index_file" ]; then
    dir_name=$(basename "$dir")
    cat <<EOF >> "$main_index_html"
        <li><a href="$base_url$dir_name">$dir_name</a></li>
EOF
  fi
done
...
#echo "Main index HTML file updated and transferred successfully."
