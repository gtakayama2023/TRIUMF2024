...
void Timing(int LT, int data){
  ...
  if(Traw_num[IP][LT][ch] < hitNmax){
    cout << "if(Traw_num[IP][LT][ch] < hitNmax) clear" << endl;
    Traw_L[IP][ch][Traw_num[IP][LT][ch]] = time;
    if(Traw_num[IP][LT][ch] == 0){ // hitN=0 -> Fill in Fiber_T
      cout << "if(Traw_num[IP][LT][ch] == 0) clear" << endl;
...
void Rawdata2root(int runN=10, bool ftree=0){
  ...
  for(int i=0;i<2;i++){
    ...
    ifname[i]=Form("../RAW/signal_HV60/MSE0000%d_192.168.10.%d.rawdata",runN,IP);
  ...
  for(int IP=0;IP<2;IP++)  rawdata[IP].close();
  timer_total.Stop();
